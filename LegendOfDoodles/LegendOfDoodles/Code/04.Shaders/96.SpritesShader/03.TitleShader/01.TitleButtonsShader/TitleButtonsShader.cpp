...
#define ExitButton m_TitleButtons[1]

////////////////////////////////////////////////////////////////////////
// 생성자, 소멸자
CTitleButtonsShader::CTitleButtonsShader(shared_ptr<CCreateMgr> pCreateMgr) : CShader(pCreateMgr)
{
	m_hWnd = pCreateMgr->GetHwnd();
}

CTitleButtonsShader::~CTitleButtonsShader()
{
}

////////////////////////////////////////////////////////////////////////
// 공개 함수
void CTitleButtonsShader::ReleaseUploadBuffers()
{
	for (int i = 0; i < 2; ++i)
	{
		if (m_TitleButtons[i])m_TitleButtons[i]->ReleaseUploadBuffers();
	}
...
void CTitleButtonsShader::UpdateShaderVariables(int opt)
{
	...
	for (int i = 0; i < 2; ++i)
	{
		...
		XMStoreFloat4x4(&pMappedObject->m_xmf4x4World,
			XMMatrixTranspose(XMLoadFloat4x4(m_TitleButtons[i]->GetWorldMatrix())));
		pMappedObject->m_percentage = (float)i;
	}
}

void CTitleButtonsShader::Render(CCamera * pCamera)
{
	...
	for (int j = 0; j < 2; j++)
	{
		m_TitleButtons[j]->Render(pCamera);
	}
}

bool CTitleButtonsShader::OnProcessMouseInput(WPARAM pKeyBuffer)
{
...
////////////////////////////////////////////////////////////////////////
// 내부 함수
D3D12_INPUT_LAYOUT_DESC CTitleButtonsShader::CreateInputLayout()
{
...
D3D12_BLEND_DESC CTitleButtonsShader::CreateBlendState()
{
...
D3D12_SHADER_BYTECODE CTitleButtonsShader::CreateVertexShader(ComPtr<ID3DBlob>& pShaderBlob)
{
...
D3D12_SHADER_BYTECODE CTitleButtonsShader::CreatePixelShader(ComPtr<ID3DBlob>& pShaderBlob)
{
	return(CShader::CompileShaderFromFile(
		L"./code/04.Shaders/99.GraphicsShader/SpriteShader.hlsl",
		"PSButtons",
...
void CTitleButtonsShader::CreateShader(shared_ptr<CCreateMgr> pCreateMgr, UINT nRenderTargets, bool isRenderBB, bool isRenderShadow)
{
	m_nPipelineStates = 1;
...
void CTitleButtonsShader::BuildObjects(shared_ptr<CCreateMgr> pCreateMgr, void * pContext)
{
	...
	m_TitleButtons[1] = pExitButton;
}

void CTitleButtonsShader::ReleaseObjects()
{
	for (int i = 0; i < 2; ++i)
	{
		if (m_TitleButtons[i]) Safe_Delete(m_TitleButtons[i]);
	}
}
